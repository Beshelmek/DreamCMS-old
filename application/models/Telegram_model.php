...
class __Module_Telegram_Sender extends CI_Model{
    ...
    private function _url($with_method = FALSE){
        ...
        $url = (TG_API_URL . $this->config->item('telegram_bot_key') .'/');
...
class Telegram_model extends CI_Model{
    ...
    private $modersID = array(
        232754916, 292645746, 220065433, 292877986
    );
    ...
    function sendInGroup($text, $type = NULL){
        $this->send->text($text, $type)->chat(-1001087692720)->send();
    }

    function sendAllModers($text, $type = NULL){
        foreach ($this->modersID as $id){
            $this->send->text($text, $type)->chat($id)->send();
        }
    }
}
